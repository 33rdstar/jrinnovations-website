...
const JRInnovationsWebsite = () => {
  ...
  const services = {
    innovation: [
      ...
      { title: 'Web Applications', desc: 'Responsive, modern websites that drive results' },
      { title: 'Mobile App Development', desc: 'iOS and Android apps that users love' },
      { title: 'IT Consulting', desc: 'Strategic technology guidance for growth' },
      { title: 'E-Commerce Solutions', desc: 'Complete online store setup and management' },
      { title: 'System Integration', desc: 'Seamless connection of your business tools' }
    ],
    creative: [
      { title: 'Brand Identity Design', desc: 'Logos that capture your essence' },
      { title: 'Graphic Design', desc: 'Eye-catching visuals for all platforms' },
      { title: 'Business Cards & Stationery', desc: 'Professional print materials' },
      { title: 'Business Profiles', desc: 'Compelling company presentations' },
      { title: 'Social Media Content', desc: 'Engaging posts and graphics' },
      { title: 'Marketing Materials', desc: 'Brochures, flyers, and promotional content' }
    ],
    entertainment: [
      { title: 'Artist Management', desc: 'Career development and representation' },
      { title: 'Event Planning & Production', desc: 'Memorable experiences, flawlessly executed' },
      { title: 'Content Creation', desc: 'Video and audio production services' },
      { title: 'Talent Booking', desc: 'Connect with the perfect performers' },
      { title: 'Brand Partnerships', desc: 'Strategic collaborations for artists' },
      { title: 'Digital Distribution', desc: 'Get your content on all platforms' }
    ]
  };

  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-white relative" style={{ scrollBehavior: 'smooth' }}>
      <AnimatedBackground />
            ...
            <div className="px-4 py-4 space-y-3">
              {['Home', 'Services', 'About', 'Contact'].map((item) => (
                <a
                  key={item}
                  href={`#${item.toLowerCase()}`}
                  className="block text-gray-700 hover:text-purple-600 font-medium transition-all duration-300"
                  onClick={(e) => handleNavClick(e, item.toLowerCase())}
                >
                  {item}
                </a>
      ...
      >
        ...
        <div className="max-w-7xl mx-auto px-4 relative z-10">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="bg-white rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-3 hover:scale-105">
          ...
          <div className="mb-16">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {services.innovation.map((service, idx) => (
                <div key={idx} className="bg-blue-50 p-6 rounded-xl shadow-md hover:shadow-2xl transition-all duration-500 border-l-4 border-blue-500 transform hover:-translate-y-2 hover:scale-105">
          ...
          <div className="mb-16">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {services.creative.map((service, idx) => (
                <div key={idx} className="bg-pink-50 p-6 rounded-xl shadow-md hover:shadow-2xl transition-all duration-500 border-l-4 border-pink-500 transform hover:-translate-y-2 hover:scale-105">
                  <h4 className="text-xl font-bold mb-2 text-gray-800">{service.title}</h4>
                  <p className="text-gray-600">{service.desc}</p>
                </div>
              ))}
          ...
          <div>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {services.entertainment.map((service, idx) => (
                <div key={idx} className="bg-green-50 p-6 rounded-xl shadow-md hover:shadow-2xl transition-all duration-500 border-l-4 border-green-500 transform hover:-translate-y-2 hover:scale-105">
                  <h4 className="text-xl font-bold mb-2 text-gray-800">{service.title}</h4>
                  <p className="text-gray-600">{service.desc}</p>
                </div>
              ))}
...
export default JRInnovationsWebsite;